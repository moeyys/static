...
read hostp
echo "开始测试中，瓜皮正在黑你服务器，会需要点时间，请稍后"
...
for i in ${arr[@]}    
do 
	wget -q --no-check-certificate https://cdn.jsdelivr.net/gh/91yun/91yuntest/test_code/${i}.sh
    . ${dir}/91yuntest/${i}.sh
	eval ${i}
done    
...
then
	...
	echo "测试结束，你服务器被瓜皮黑了，具体日志查看 91yuntest.log"
fi
#删除目录
rm -rf ${dir}/91yuntest
